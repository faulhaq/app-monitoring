...
class HarianYn extends Model
{
    ...
    public static function apply_kelas($id_data, ?array $tujuan_kelas)
    {
        ...
        return DB::table("data_harian_yn_kelas")->insert($data);
    }

    public static function get_tujuan_kelas($id_data)
    {
        $data = DB::table("data_harian_yn_kelas")
                ->where("id_data", $id_data)
                ->get();
        $ret = [];
        foreach ($data as $v) {
    ...
    public function get_list_kelas_view()
    {
        ...
        $data = DB::table("kelas")
                ->select("kelas.*")
                ->join("data_harian_yn_kelas", "kelas.id", "data_harian_yn_kelas.id_kelas")
                ->where("data_harian_yn_kelas.id_data", $this->id)
