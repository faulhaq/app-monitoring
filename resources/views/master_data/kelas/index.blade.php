                ...
                <div class="col-md">
                    <div class="form-group">
                        ...
                        <select id="filter_tahun_ajaran" name="filter_tahun_ajaran" class="select2bs4 form-control">
                            <option value="all">Semua Tahun Ajaran</option> @foreach ($tahun_ajaran as $v) <?php $active = $id_tahun_ajaran_aktif == $v->id ? " (aktif)" : ""; ?> <?php $sel = $v->id == $ftahun_ajaran ? " selected" : ""; ?> <option value="{{ $v->id }}" {!! $sel !!}>{{ $v->tahun.$active }}</option> @endforeach
                        </select>
                    </div>
                </div>
            </div>
            <table id="example1" class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td> <?php $enc_id = Crypt::encrypt($data->id); ?> <form action="{{ route('kelas.destroy', $enc_id) }}" method="post"> @csrf @method('delete') <a href="{{ route('kelas.kelola', $enc_id) }}" class="btn btn-info btn-sm mt-2">
                                    <i class="nav-icon fas fa-id-card"></i> &nbsp; Kelola </a> @if ($role === "admin") <a href="{{ route('kelas.edit', $enc_id) }}" class="btn btn-success btn-sm mt-2">
                                    <i class="nav-icon fas fa-edit"></i> &nbsp; Edit </a> @endif @if ($role === "admin") <button class="btn btn-danger btn-sm mt-2">
                                    <i class="nav-icon fas fa-trash-alt"></i> &nbsp; Hapus </button> @endif </form>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select id="tingkatan" name="tingkatan" class="select2bs4 form-control @error('tingkatan') is-invalid @enderror">
                                <option value="">-- Pilih Kelas --</option> @for ($i = 1; $i <= 6; $i++) <option value="{{ $i }}">{{ $i }}</option> @endfor
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select id="tahun_ajaran" name="tahun_ajaran" class="select2bs4 form-control @error('tahun_ajaran') is-invalid @enderror">
                                <option value="">-- Pilih Tahun Ajaran --</option> @foreach ($tahun_ajaran as $v) <option value="{{ $v->id }}">{{ $v->tahun }}</option> @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="wali_kelas">Wali Kelas</label>
                            <select id="wali_kelas" name="wali_kelas" class="select2bs4 form-control @error('wali_kelas') is-invalid @enderror">
                                <option value="">-- Pilih Wali Kelas --</option> @foreach ($guru as $v) <option value="{{ $v->id }}">({{ $v->nik }}) {{ $v->nama }}</option> @endforeach
                            </select>
                        </div>
                    </div>
                </div>
