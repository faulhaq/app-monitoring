...
@section('content')
    ...
    <div class="col-md-12">
                                ...
                                @foreach ($list_kelas as $v)
                                    ...
                                    @if ($ta->id == $id_tahun_ajaran_aktif)
                                        ...
                                        <option value="{{ $v->id }}"{!! $sel !!}>
                                            {{ $v->tingkatan . $v->nama . " (TA: {$ta->tahun})" }} (aktif)</option>
                                    @endif
                                @endforeach
                                @foreach ($list_kelas as $v)
                                    <?php $ta = $v->tahun_ajaran(); ?>
                                    @if ($ta->id != $id_tahun_ajaran_aktif)
                                        <?php $sel = $v->id == $fkelas ? ' selected' : ''; ?>
                                        <option value="{{ $v->id }}"{!! $sel !!}>
                                            {{ $v->tingkatan . $v->nama . " (TA: {$ta->tahun})" }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    @if (!empty($fsiswa))
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="date" id="ftanggal" name="ftanggal" value="{{ $ftanggal }}"
                                    class="form-control @error('ftanggal') is-invalid @enderror">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <a
                                    href="{{ route('monitoring.harian.orang_tua.calendar', urlencode(Crypt::encrypt($siswa->id))) . '?tahun=' . $tahun . '&bulan=' . $bulan . '&kelas=' . $fkelas }}">
                                    <button type="button" id="lihat_kalender" class="btn btn-primary form-control">Lihat
                                        Kalender</button>
                                </a>
                            </div>
                        </div>
        ...
        @if (!empty($fsiswa))
            <form
                action="{{ route('monitoring.harian.terima_jawaban') . '?id_siswa=' . $siswa->id . '&tanggal=' . $ftanggal }}"
                method="POST">
                @csrf
                <div class="card card-primary">
                    <div class="card-header">
                        ...
                        @if (count($pertanyaan) > 0)
                            <div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <p>Catatan: Jika tombol "Simpan" ditekan, maka orang tua tidak dapat mengubah jawaban
                                        dan point akan ditampilkan di sisi orang tua.</p>
                                </div>
                            </div>
                        @else
                            <div>
